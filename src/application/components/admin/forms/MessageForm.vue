<template>
	<form class="mx-2" @submit.prevent="submit">
		...
		<div class="form-group my-3">
			...
			<input class="form-control" placeholder="Eg. Book an appointment" v-model="factory.subject" id="subject"
			       :class="{'is-invalid': factory.errors.subject, 'is-valid': factory.isValid('subject')}">
			<span class="small" v-if="factory.errors.subject">{{ factory.errors.subject }}</span>
		</div>
		<div class="form-group my-3">
			...
			<textarea class="form-control" v-model="factory.message" id="message" rows="6"
		          ...
		          placeholder="Eg I want to book a private session with the pastor on Friday at 12 noon">
			</textarea>
...
import { MessageFactory } from '@modules/forms/domain/factories/message'
